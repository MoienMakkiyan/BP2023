...
void draw_ball(SDL_Renderer *Renderer,int L,int W,int r,int n_line)
{
    ...
    for(int i=0;i<n_line;i++)
    {
        for(int j=0;j<n_colum;j++)
        {
            filledCircleRGBA(Renderer,balls[i*n_colum+j].x,balls[i*n_colum+j].y,r,balls[i*n_colum+j].color_r,balls[i*n_colum+j].color_g,balls[i*n_colum+j].color_b,255);
        }
    }
}
...
void crashed_ball(SDL_Renderer *Renderer)
{
    ...
    if (!is_crash_ball_crashed)
    {
        if(!is_crash_ball_moved){
            ...
            {
                if(((crash_ball.x-balls[i].x)*(crash_ball.x-balls[i].x)+(crash_ball.y-balls[i].y)*(crash_ball.y-balls[i].y))<4*rad_ball*rad_ball)
                {
                    cout<<"####"<<endl;
